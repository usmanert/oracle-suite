...
=======
const gsu2URL = "https://api1.gsucoin.app/Products/GSULive/?symbol=%s"
>>>>>>> 5b8bb2c4336af0cb37aa0dcdd6f8d4de253cc5e8
...
// GSU2 exchange handler
type GSU2 struct {
        WorkerPool query.WorkerPool
}

func (h *GSU2) localPairName(pair Pair) string {
        return strings.ToUpper(pair.Base)
}

func (h *GSU2) getURL(pairs []Pair) string {
        pairsStr := make([]string, len(pairs))
        for i, pair := range pairs {
                pairsStr[i] = h.localPairName(pair)
        }
        return fmt.Sprintf(gsu2URL, strings.Join(pairsStr, ","))
}

func (h GSU2) Pool() query.WorkerPool {
        return h.WorkerPool
}

func (h GSU2) PullPrices(pairs []Pair) []FetchResult {
...
func (h *GSU2) fetch(pairs []Pair) ([]FetchResult, error) {

        crs := make([]FetchResult, len(pairs))
        respMap := map[string]gsu2Response{}
        for i, pair := range pairs {
                symbol := h.localPairName(pair)
                url := fmt.Sprintf(gsu2URL, symbol)
                ...
                // parsing JSON
                var resp gsu2Response
                err := json.Unmarshal(res.Body, &resp)
                if err != nil {
                        return nil, fmt.Errorf("failed to parse GSU2 response: %w", err)
                }
                respMap[symbol] = resp
                ...
                if resp, has := respMap[pair.Base]; has {
                        ...
                        if err != nil {
                                crs[i] = fetchResultWithError(
                                        pair,
                                        fmt.Errorf("failed to create price point from GSU2 response: %w: %s", err, res.Body),
                                )
                        } else {
                                crs[i] = fetchResult(p)
                        }
                        ...
                        crs[i] = fetchResultWithError(
                                pair,
                                fmt.Errorf("failed to find symbol %s in GSU2 response: %s", pair, res.Body),
                        )
                }
        }
        return crs, nil
...
func (h *GSU2) newPrice(pair Pair, resp gsu2Response) (Price, error) {
